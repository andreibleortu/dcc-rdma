...
int lambda_run_client(const char *server_name)
{
    ...
    // Setup control QP for Send/Recv
    DEBUG_LOG("Setting up control QP");
    if (setup_rdma_connection(&config.ctrl_qp, server_name, MODE_SEND_RECV, NULL) != RDMA_SUCCESS) {
        fprintf(stderr, "Failed to setup control QP connection\n");
        cleanup_resources(&config.data_qp);
        return -1;
    }

    // Set up signal handler for Ctrl+C
    global_config = &config.ctrl_qp;  // global_config is now struct config_t*
